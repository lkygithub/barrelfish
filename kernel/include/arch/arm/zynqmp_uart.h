...
#define ZYNQMP_UART_MAX_PORTS 2
 ...
 * enabled the OS should then call zynqmp_uart_init below. 
 */
extern void zynqmp_uart_early_init(unsigned port, lpaddr_t addr);
 ...
 */
extern void zynqmp_uart_init(unsigned port, lvaddr_t base, bool hwinit);

/*
 * \brief Put a character to the port
 */
extern void zynqmp_uart_putchar(unsigned port, char c);

/*
 * \brief Read a character from a port
 */
extern char zynqmp_uart_getchar(unsigned port);
